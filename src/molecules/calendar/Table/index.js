...
import EventContainer from '../../../atoms/calendar/EventContainer';
...
const CalendarTable = ({
    month, selected, events, calendars = [], onDrop
}) => {
    ...
    while (day <= endDate) {
        ...
        for (let i = 0; i < 7; i++) {
            days.push(
                <EventContainer { ...({
                    current, monthStart, selected, monthEnd, day, events: normalizedEvents, index: i
                }) }
                        onDrop={ onDrop }
                        key={ i } />
            );
